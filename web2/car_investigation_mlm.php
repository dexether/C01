...
$_SESSION['page'] = 'car_investigation_mlm';
/**
 * 
 * Start Coding
 */
...
AND marketing.`email`= client_accounts.`email`
...
$accnobymlm0  = '';
foreach($row as $rows) {
    $accnobymlm = $rows['accountname'];
    $accnobymlm0 = $rows['group_city'];
}
$template->assign("accnobymlm", $accnobymlm);
$template->assign("accnobymlm0", $accnobymlm0);
// var_dump($accnobymlm);
// var_dump($accnobymlm0);

$query = "SELECT 
  branch_manager.`email` 
FROM
  branch_manager 
WHERE branch_manager.`group_branch` = '$accnobymlm0' ";
$row = $DB->execresultset($query);
// var_dump($query);
$bmnya  = '';
foreach($row as $rows) {
 ...
 * End Of coding
 */
$template->display("car_investigation_mlm.htm");
