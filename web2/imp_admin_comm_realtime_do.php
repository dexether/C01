    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if ($security->get($token)) {
            ...
            if ($postmode == 'tmp') {
                ...
                if ($account == '0') {
                    # code...
                    $query  = "SELECT * FROM mlm_realtime WHERE unix = '$unix' AND is_compress = TRUE";
                    $result = $DB->execresultset($query);
                    foreach ($result as $key => $value) {
                        # code...
                        hitungkomisi($value['ACCNO'], $periode_date, $periode);
                    }
                } else {
...
function getDataAll($unix)
{
    global $DB;
    global $user;
    $query  = "SELECT ACCNO, Upline FROM mlm WHERE ACCNO <> 'COMPANY'";
    $result = $DB->execresultset($query);
    $data   = array();
    foreach ($result as $key => $value) {
        # code...
