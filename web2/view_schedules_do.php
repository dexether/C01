	...
	$ceks = array();
		$query	  = "SELECT 
					...
					FROM
					  SCHEDULE,
					  client_aecode 
					WHERE SCHEDULE.`email` = client_aecode.`aecode` 
					AND SCHEDULE.`date_meeting` = '$prodate'
					ORDER BY schedule.`nameaccount` ASC  ";
		$rows = $DB->execresultset($query);
		foreach ($rows as $row) {
		$ceks[] = $row;
		}
	 ...
	 $ceks = array();
		$query	  = "SELECT 
				...
				FROM
				  ...
				  client_aecode 
				WHERE SCHEDULE.`email`= client_aecode.`aecode`
				AND schedule.`nameaccount` = mlm.`ACCNO` 
				AND SCHEDULE.`date_meeting` = '$prodate'
				AND mlm.branch ='$view'";
		$rows = $DB->execresultset($query);
		foreach ($rows as $row) {
...
/*=====  End of Coding  ======*/
$template->display("view_schedules.htm");
