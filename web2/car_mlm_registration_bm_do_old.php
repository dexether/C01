...
$query = "SELECT 
			...
			FROM
			  client_aecode 
			WHERE client_aecode.`aecode` = '$marem' ";
   
     $rows = $DB->execresultset($query);
	$aecodeidnya="";
	 foreach ($rows as $row) {
         $aecodeidnya = $row['aecodeid'];
    }
    $template->assign("aecodeidnya", $aecodeidnya);
	// var_dump($aecodeidnya);
    ...
    if ($postmode == "createnewlevel2") {
        ...
        if ($adaae != 'adaae' && $email != 'admin@si.co.id') {
            ...
            if ($email == 'admin@si.co.id') {
                $upline = 'COMPANY';
					...
					FROM
					  mlm,
					  client_accounts
					WHERE mlm.`Upline` = client_accounts.`accountname` 
					AND client_accounts.`email` = '$email' LIMIT 1";
            ...
            $query = "SELECT client_branch.branch,client_group.group AS thegroup,
                ...
                and client_aecode.aecode = '$marem'
                ";
