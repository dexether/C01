...
class Imap {
    ...
    public function getMessages($type = 'text') {
        ...
        $emails = imap_search($stream, 'UNSEEN FROM "d@sentratamainvestorfuture.net"');
        $messages = array();
        if ($emails) {
            $this->emails = $emails;
            $i = 0;
    ...
    private function loadMessage($uid, $type) {
        ...
        $array['message'] = imap_body($this->imapStream,$uid,FT_UID);
    ...
    //     $mbox = $this->imapStream;
    //     $result = $this->emails;
    //     if ($result == null) {
    //         # code... jika tidak ada email yang belum terbaca
    //     } else {
