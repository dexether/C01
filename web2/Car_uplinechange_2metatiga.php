...
$_SESSION['page'] = 'car_uplinechange_2metatiga';
/*==============================
=        Start Coding          =
==============================*/
$metanya = "";
if (isset($_GET['metaac'])) {
	$metanya = $_GET['metaac'];
}
...
$query = "SELECT 
		  client_accounts.`aecodeid`,
		  client_accounts.`email`
		FROM
		  client_accounts
		WHERE client_accounts.`accountname`='$metanya'";
		$accmeta = array();
		$accmeta0 = array();
		$rows = $DB->execresultset($query);
foreach ($rows as $row) {
    $accmeta = $row['aecodeid'];
    $accmeta0 = $row['email'];
}
$template->assign("accmeta", $accmeta);
$template->assign("accmeta0", $accmeta0);
// var_dump($accmeta);
// var_dump($accmeta0);
		
/*=====  End of Coding  ======*/
$template->display("car_uplinechange_2metatiga.htm");
