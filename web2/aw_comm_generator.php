    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if ($security->get($token)) {
            ...
            if ($postmode == 'mlm_temp') {
                // Check it first
                $query  = "SELECT ACCNO, datetime FROM mlm_temp WHERE type = 'asiawide' AND rolldate = '$periode_date' LIMIT 0 ,1";
                ...
                if (count($result) > 0 && $replace == 'off') {
                    ...
                    $delete = "DELETE FROM mlm_temp WHERE rolldate = '$periode_date' AND type = 'asiawide'";
                    $DB->execonly($delete);

                    $delete = "DELETE FROM mlm_comm WHERE rolldate = '$periode_date' AND type = 'asiawide'";
                    $DB->execonly($delete);
                  ...
                  WHERE mlm.`ACCNO` = client_accounts.`accountname`
                    ...
                    AND ACCNO != 'COMPANY'
                    AND mlm.`group_play` LIKE '%asiawide%'";
                    $result = $DB->execresultset($query);
                ...
                // Check it first
                $query  = "SELECT ACCNO FROM mlm_comm WHERE rolldate = '$periode_date' AND type = 'asiawide' LIMIT 0,1 ";
                $result = $DB->execresultset($query);
                if (!count($result) > 0) {
                    $query = "SELECT
                    mlm_temp.`ACCNO`,
                    ...
                    FROM
                      ...
                      WHERE mlm_temp.`type` = 'asiawide'
                      GROUP BY mlm_temp.`ACCNO`
                    ORDER BY mlm_temp.`ACCNO`
                    AND mlm_temp.`type` = 'asiawide'";
                    $result = $DB->execresultset($query);
              ...
              $query = "SELECT id FROM mlm_ewallet_moving WHERE ref_number = 'asiawide_commision:".$periode_date."'";
              $result = $DB->execresultset($query);
              if (count($result) > 0) {
                # code...
                $error    = "warning";
...
function compression($account, $base_account, $periode_date)
{
    ...
    if ($typeofaccounts != 'agent' && $Upline != 'COMPANY') {
        ...
        if ($downline > 1 && $lots >= 1) {
            ...
            $query  = "SELECT ACCNO FROM mlm_temp WHERE ACCNO = '$base_account' AND rolldate = '$periode_date' AND type = 'asiawide'";
            $result = $DB->execresultset($query);
            if (count($result) > 0) {
                # code...
            } else {
                $insert = "INSERT INTO mlm_temp SET ACCNO = '$base_account', Upline = '$Upline', datetime = NOW(), methode = 'compress', rolldate = '$periode_date', type = 'asiawide'";
                $DB->execonly($insert);
            }
        } else {
        ...
        if (count($result) > 0) {
            ...
            if ($account == $base_account) {
                ...
                $insert = "INSERT INTO mlm_temp SET ACCNO = '$base_account', Upline = '$account', datetime = NOW(), methode = 'compress', rolldate = '$periode_date', type = 'asiawide'";
                $DB->execonly($insert);
            }
        ...
        if ($element['upline'] == $parentId) {
            ...
            if ($children) {
                if ($element['downline'] <= 1) {
                    ...
                    $insert = "INSERT INTO mlm_temp SET ACCNO = '$element[ACCNO]', Upline = '$element[upline]',  methode = 'general', rolldate = '$periode_date', type = 'asiawide'";
                    $DB->execonly($insert);

                    $query = "SELECT ACCNO, id FROM mlm_temp WHERE ACCNO = '$row[ACCNO]' AND rolldate = '$periode_date' AND type = 'asiawide'";
                    $res   = $DB->execresultset($query);
                    if (count($res) > 0) {
                        $update = "DELETE FROM mlm_temp WHERE id = '" . $res[0]['id'] . "'";
                        $DB->execonly($update);
                        $insert = "INSERT INTO mlm_temp SET ACCNO = '$row[ACCNO]', Upline = '$element[upline]',  methode = 'compress', rolldate = '$periode_date', type = 'asiawide'";
                        $DB->execonly($insert);
                    } else {
                        $insert = "INSERT INTO mlm_temp SET ACCNO = '$row[ACCNO]', Upline = '$element[upline]',  methode = 'compress', rolldate = '$periode_date', type = 'asiawide'";
                        $DB->execonly($insert);
                    ...
                    $query = "SELECT ACCNO FROM mlm_temp WHERE ACCNO = '$element[ACCNO]' AND rolldate = '$periode_date' AND type = 'asiawide'";
                    $res   = $DB->execresultset($query);
                    if (count($res) > 0) {
                ...
                if (count($res2) > 0) {
                    ...
                    $insert = "INSERT INTO mlm_temp SET ACCNO = '$element[ACCNO]', Upline = '$element[upline]',  methode = 'general2', rolldate = '$periode_date', type = 'asiawide'";
                    $DB->execonly($insert);
                }
            }
...
AND mlm_temp.type = 'asiawide'";
    ...
    if ($typeaccount != 'agent') {
        if ($cektrade >= 1 && $checkdownline >= 2) {
            ...
            $insert = "INSERT INTO mlm_comm SET type = 'asiawide', ACCNO = '$account', mlm_comm.from = '$account', level = '1', amount = '$subcomm', lot = '$sublot', rolldate = '$periode_date'";
            $DB->execonly($insert);
    ...
    foreach ($result as $row) {
        ...
        if ($is_agent) {
            if (!($level > 7) && $commision != 0) {
                ...
                $insert = "INSERT INTO mlm_comm SET type = 'asiawide', ACCNO = '$parent', mlm_comm.from = '$accno', level = '$level', amount = '$commision', lot = '$cektrade', rolldate = '$periode_date'";
                $DB->execonly($insert);
            }
        } else {
...
// SELECT
    //   SUM(volume/100)
    // FROM
    //   asiawide_source_mini.`mt4_trades`
...
function ewallets($account, $amount, $desc){
  ...
  if ($id_wallet != '0') {
    ...
    $insert = "INSERT INTO mlm_ewallet_moving SET id_ewallet = '$id_wallet', amount = '$amount', cmd = '3', reason = '$desc', ref_number = 'asiawide_commision:".$periode_date."'";
    $DB->execonly($insert);
    $output = TRUE;
  }else{
