...
$query = "SELECT table_name FROM information_schema.tables WHERE table_schema='trado_real_logs';";
$rows = $DB->execresultset($query);
foreach ($rows as $row) {
...
$query = "SELECT table_name FROM information_schema.tables 
    WHERE table_schema='trado_real_logs' and table_name between ('logs_$datesearch1') and ('logs_$datesearch2') ";
...
if (count($rolldatas) > 0) {
    for ($icount = 0; $icount < count($rolldatas); $icount++) {
        ...
        $query = "SELECT id,code,ip,msg,ctm,unused1,unused2 FROM trado_real_logs.logs_$datameta_rollover 
