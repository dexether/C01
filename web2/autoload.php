...
$query = "SELECT resetuser.email,resetuser.keterangan,resetuser.refresh,
    resetuser.modify_time,resetuser.table  
    FROM 
    resetuser,client_aecode 
    WHERE resetuser.`email` = client_aecode.aecode 
    AND client_aecode.aecode = '$user->username' order by modify_time asc 
        ";
//tradeLogAutoLoad("AutoLoad-126:" . $query);
$messages = array();
...
$title = 2;
$keterangan='';
for ($icount = 0; $icount < count($messages); $icount++) {
    $message = $messages['$icount'];
    $_SESSION['page'] = $message['table'];
    if ($message['refresh'] == 'yes') {
        $title = 1;
    }
    if ($keterangan == '') {
        $keterangan = $message['keterangan'];
    } else {
        $keterangan = $keterangan . "<br>" . $message['keterangan'];
    }


    $query = "delete from resetuser where modify_time = '$message[modify_time]' and email = '$message[email]'";
    $rows = $DB->execonly($query);
}

echo $title . ";" . $keterangan;

exit;

function tradeLogAutoLoad($msg) {
