...
$query = "SELECT accountname, client_aecode.name FROM client_accounts, client_aecode WHERE client_accounts.aecodeid = client_aecode.aecodeid AND client_aecode.aecodeid = '$users' AND client_accounts.typeaccount = 'agent'";
$result = $DB->execresultset($query);
if (count($result) > 0) {
   $error    = "error";
