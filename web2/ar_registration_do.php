...
$accnomlm = '';
if (isset($_GET['accnomlm'])) {
    $accnomlm = strtoupper($_GET['accnomlm']);
}
...
if ($_GET['postmode']) {
    $postmode = $_GET['postmode'];
    if ($postmode == "postmode") {
        $update_tradeby = $user->getUsername();
        $plan           = anti_injection($_GET['plan']);
        $accnomlm       = anti_injection(strtoupper($_GET['accnomlm']));
        $hp             = anti_injection($_GET['hp']);
        ...
        AND client_accounts.`accountname` = '$accnomlm'
        ...
        if ($adaae != 'adaae' && $accnomlm != 'COMPANY') {
           ...
           ";
            ...
            if ($accnomlm == 'COMPANY') {
                $query = "SELECT
            mlm_bonus_settings.`id`,
            mlm_bonus_settings.`group_play`,
            mlm_bonus_settings.`mlmaccount`
            FROM
            mlm_bonus_settings
            WHERE mlm_bonus_settings.`group_play` = '$plan'";
                $res      = $DB->execresultset($query);
                $accnomlm = $res[0]['mlmaccount'];
            }
        ...
        ";
            //tradeLogMMNewLevel("tradeLogMMNewLevel-800:" . $query);
            $DB->execonly($query);
            $query = "insert into mlm_payment set
        aecode = '$user->username',
        Account = '$accountnamebaru',
        Amount = '0',
        ...
        ";
            ...
            $query = "INSERT INTO mlm_ewallet SET
        aecode = '$usernya[aecodeid]',
        account = '$accountnamebaru',
        balance = '0',
        balance_prev = '0',
        lastupdate = NOW(),
        ...
        ";
            $DB->execonly($query);
            $query = "INSERT INTO mlm_goldsaving SET
        aecode = '$usernya[aecodeid]',
        account = '$accountnamebaru',
        balance = '0',
        balance_prev = '0',
        ...
        ";
            ...
            $query = "INSERT INTO mlm_wcd SET account = '$accountnamebaru', last_pay ='0000-00-00', next_pay = '0000-00-00', status = '0'";
            $DB->execonly($query);
