<?php
include_once("$_SERVER[DOCUMENT_ROOT]/includes/functions.php");
include_once("$_SERVER[DOCUMENT_ROOT]/classes/Manager.class.php");
include_once("includes/wr_tools.php");
$var_to_pass = null;
global $user;
global $template;
global $themonth;
global $mysql;
global $DB;

if (isset($user)) {
    $user;
}
$user = $_SESSION['user'];
$template->assign("user", $user);

include_once("$_SERVER[DOCUMENT_ROOT]/classes/FetchAccount.class.php");
$theFetchAccount = new theOtherFetchAccounts();
$cabang_admin = 'semua';
$accounts = $theFetchAccount->fetchAccountslangsung($user,$mysql['meta'],$cabang_admin);

$lines = "a=1";
if ($mysql['crypt_key'] != '') {
    $crypt_key = $mysql['crypt_key'];
}

$key = '';
if (isset($_SESSION['key'])) {
    $key = $_SESSION['key'];
}

$tools = new CTools();
$data = base64_decode(str_replace(array('123', ','), array('+', '/'), $key));
$data = explode("\n", gzuncompress($tools->Crypt($data, $crypt_key)));
$variabel = explode("&", $data[0]); //a=1&account=802222&postmode=deposit&tradedby=ALBERTOSCARINA
$accountlink = $variabel[1]; //account=1234567
$accountvariabel = explode("=", $accountlink);
$account = $accountvariabel[1];
$lines = $lines . "&account=" . $account;
$linezip = gzcompress($lines);
$key = str_replace(array('+', '/'), array('123', ','), rtrim(base64_encode($tools->Crypt($linezip, $crypt_key)), '='));
$_SESSION['key'] = $key;

$accountcheck = myfilter($accounts, $account);
if ($accountcheck[0] == '') {
    display_error("107.You do not have permission to access this page.<br>If you feel this is an error, please contact the Administrator.");
} else {
    if (!empty($account) && $account != "all") {
        if (!in_array($account, $accounts)) {
            //TradeLogUnderConstruct_Secure("Profile-111");
            display_error("111.You do not have permission to access this page.<br>If you feel this is an error, please contact the Administrator.");
        }
    }
    //TradeLogUnderConstruct_Secure("Profile-115");
    $template->assign("accounts", $accounts);
    if (!empty($account)) {
        $template->assign("tradedby", $user->username);
        $template->assign("account", $account);
        $template->assign("error", "");
    }
}
$template->assign("account", $account);

//TradeLogUnderConstruct_Secure("Profile-175-Get_PostMode:" . $_GET[postmode]);
$postmode = '';
if (isset($_GET['postmode'])) {
    $postmode = $_GET['postmode'];
}

$_SESSION['page'] = 'group_account';
/**
 * 
 * Start Coding
 */

$query = "SELECT ACCNO FROM mlm WHERE updateby = '".$user->username."'";
$do = $DB->execresultset($query);
foreach($do as $rows) {
    $accnobymlm[] = $rows['ACCNO'];
}
$template->assign("accno", $accnobymlm);

$template->assign("groupid", $user->groupid);
$clientaecode = array();
if ($user->groupid == '3') {
    $query = "SELECT client_aecode.*   
        from client_aecode 
        where 
        client_aecode.aecode = '$user->username'";
    //TradeLog_MLMRegistration("MLM_Registration-79=" . $user->groupid . ";" . $query);
    $rows = $DB->execresultset($query);
    foreach ($rows as $row) {
        $clientaecode = $row;
    }
    $template->assign("clientinfo", $clientaecode);
}else{
    $template->assign("clientinfo", $clientaecode);
}

$cari="";
if(isset($_GET['account'])){
    $cari = $_GET['account'];
}

$template->assign("cari", $cari);


$cari1="";
if(isset($_GET['ACCNO'])){
    $cari1 = $_GET['ACCNO'];
}

if(isset($_GET['account'])){
    $cari = $_GET['account'];
}else{
	$cari = $cari1;
}
$pialang = array();
if(isset($_POST['kot'])) {
        $pialang = $_POST['kot'];
    }
$pipi="";	
foreach ($pialang as $pialang) {
        $pipi = $pialang;
    }
 
$accnoss = array();
$query = "SELECT 
  client_aecode.name
FROM
  client_aecode,
  client_accounts,
  mlm 
WHERE client_aecode.`aecodeid` = client_accounts.`aecodeid` 
  AND client_accounts.`accountname` = mlm.`ACCNO`
  AND mlm.`ACCNO` = '$cari'";
 $rows = $DB->execresultset($query);
 $accnoss = "";
foreach($rows as $row) {
	$accnoss = $row['name'];
}
$template->assign("accnoss", $accnoss);
$mtaccount = array();
 $query	  = "SELECT 
  mlm.mt4dt 
 FROM
   mlm 
 WHERE mlm.`ACCNO` = '$cari1' ";
 $rows = $DB->execresultset($query);
 $mtaccount = "";
foreach($rows as $row) {
$mtaccount = $row['mt4dt'];
}
$template->assign("mtaccount", $mtaccount);
 
 if ($pipi=="yes") {
	 $condisional= "";
 }else{
	 $condisional= "and mlm.mt4dt = '$mtaccount'";
 }

$statements = array();
$query	  = "SELECT 
  branch,
  mlm.* 
FROM
  client_aecode,
  client_accounts,
  mlm,
  acc_kota 
WHERE client_aecode.`aecodeid` = client_accounts.`aecodeid` 
  AND client_accounts.`accountname` = mlm.`ACCNO` 
  AND mlm.`mt4login` = acc_kota.`login` 
  AND mlm.`mt4dt` = acc_kota.`mt4dt` 
  AND mlm.`Upline` = '$cari1'
  $condisional";
$rows = $DB->execresultset($query);
foreach ($rows as $row) {
$statements[] = $row;
}
$template->assign("cari", $cari);
$template->assign("accnoss", $accnoss);
$template->assign("statements", $statements);

/**
 *
 * End Of coding
 */
$template->display("group_account.htm");
function myfilter($input_var_outer, $param) {
    global $var_to_pass;
    $var_to_pass = $param;

    function mycallback($input_var_inner) {
        global $var_to_pass;
        return ($input_var_inner == $var_to_pass) ? true : false;
    }

    $return_arr = array_filter($input_var_outer, 'mycallback');
    $return_arr = array_merge(array(), $return_arr);
    return $return_arr;
}

function TradeLogUnderConstruct_Secure($msg) {
    $fp = fopen("trader.log", "a");
    $logdate = date("Y-m-d H:i:s => ");
    $msg = preg_replace("/\s+/", " ", $msg);
    fwrite($fp, $logdate . $msg . "\n");
    fclose($fp);
    return;
}


?>