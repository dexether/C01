           ...
           ";
            ...
            $_SESSION['page'] = 'treview';
               ...
               # code...
				...
				$query = "UPDATE user SET groupid = '6' WHERE username = '$username'";
				$DB->execonly($query);
				
				$query  = "SELECT accountname, client_aecode.name FROM client_accounts, client_aecode, mlm WHERE client_accounts.aecodeid = client_aecode.aecodeid AND client_accounts.accountname = mlm.ACCNO AND client_aecode.aecodeid = '$users' AND mlm.group_play = '$type'";
				$result = $DB->execresultset($query);
				if (count($result) > 0) {
					foreach($result as $row){
						$accname = $row['accountname'];
						$accnameib = $row['accountname'].'-IB';
						$query = "UPDATE client_accounts SET typeaccount = 'agent', accountname = '$accnameib', name = '$accnameib' WHERE accountname = '$accname'";
						//tradeLogMMNewLevel("tradeLogMMNewLevel-800:" . $query);
						$DB->execonly($query);
						$query = "UPDATE mlm SET ACCNO = '$accnameib' WHERE ACCNO = '$accname'";
						//tradeLogMMNewLevel("tradeLogMMNewLevel-800:" . $query);
						$DB->execonly($query);
					}
					$error            = "success";
					$subject          = "The cabinet ID " . $accname . " has been upgraded to Agent ";
					$msg              = "";
				}else{
					$query = "INSERT INTO client_accounts SET " .
                    "aecodeid = '" . $users . "', " .
