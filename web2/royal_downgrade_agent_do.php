           ...
           ";
            ...
            $_SESSION['page'] = 'treview';
               ...
               # code...
				...
				$query = "SELECT accountname FROM client_accounts WHERE aecodeid = '$users'";
                //tradeLogMMNewLevel("tradeLogMMNewLevel-800:" . $query);
                $rows      = $DB->execresultset($query);
				
				tradeLogMMNewLevel("username :" . $username);
				$query = "UPDATE user SET groupid = '3' WHERE username = '$username'";
				$DB->execonly($query);
				
				foreach ($rows as $row) {
					$accountname = $row['accountname'];
					
					$query = "SELECT * FROM mlm WHERE Upline = '$accountname'";
					$rows      = $DB->execresultset($query);
					foreach ($rows as $row) {
						$accno = $row['ACCNO'];					
						$query = "UPDATE mlm SET Upline = 'COMPANY', updateby = '$update_tradeby' WHERE ACCNO = '$accno'";
						$DB->execonly($query);
						$userdatas = getIdentitas($accno);
						$timenya   = date('Y-m-d H:i', strtotime('-1 hour'));
						$subject   = "Your Cabinet ID $accno was Downgraded By Admin to reguler account";
						$body      = "Time: " . $timenya . "<br> <br>";
						$body      = $body . "Dear  $userdatas[name],<br>";
						$body      = $body . " <br>";
						$body      = $body . "Your Cabinet ID has been downgraded. this Cabinet ID is downgraded by Administrator Privileges<br>";
						$body      = $body . " <br>";
						$body      = $body . "Thank you," . "<br>";
						$body      = $body . "<br><strong>" . $companys['companyname'] . "</strong>" . "<br>";
						$body      = $body . $companys['long_address'];
					...
					$accno = explode('-',$accountname);
					$accnobaru = $accno[0];
					$query = "UPDATE client_accounts SET accountname = '$accnobaru', name = '$accnobaru', typeaccount = 'reguler' WHERE accountname = '$accountname'";
					$DB->execonly($query);
					$query = "UPDATE mlm SET ACCNO = '$accnobaru' WHERE ACCNO = '$accountname'";
					$DB->execonly($query);
				}
				
				$error            = "success";
				$subject          = "The Agent with cabinet ID " . $accountnamebaru . " has been downgraded ";
				$msg              = "";
				$link             = $companys['appurl'] . "/web2/mainmenu.php";
				// $_SESSION['page'] = 'tw';
