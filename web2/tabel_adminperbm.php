...
$_SESSION['page'] = 'tabel_adminperbm';
/*==============================
=        Start Coding          =
==============================*/

$downline = "";
if (isset($_GET['mailbm'])) {
	$downline = $_GET['mailbm'];
}
...
/*  Cari Downline */
    ...
    $condiional = "AND client_aecode.aecode = '$downline'";
    $condiional_header = "''";
    $condiional_footer = "";
...
$query = "SELECT 
		 metode_comisi.`metode`
		FROM
		  metode_comisi,
		  branch_manager
		WHERE branch_manager.`group_branch`= metode_comisi.`group_branch`
		AND  branch_manager.`email`='$downline'";
   
     $rows = $DB->execresultset($query);
		$metodenya= array();
	 foreach ($rows as $row) {
         $metodenya = $row['metode'];
    }
    $template->assign("metodenya", $metodenya);
	// var_dump($metodenya);
...
foreach ($multiarray as $key => $test1){
	...
	$jmlcom = $jmlcom + $tes2['lot_meta']*$tes2['comisi'] ;
	}
	// $datas[$i2]['nama'] = $nama;
...
/*=====  End of Coding  ======*/
$template->display("tabel_adminperbm.htm");
