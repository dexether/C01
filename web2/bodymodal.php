...
$query	  = "SELECT 
			...
			FROM
			  mar_ac 
			WHERE mar_ac.`datetime` = '$lihat'";
		$rows = $DB->execresultset($query);
		foreach ($rows as $row) {
		$tampil1 = $row['datetime'];
		$tampil2 = $row['account'];
		$tampil3 = $row['pros_date'];
		$tampil4 = $row['pros_name'];
		$tampil5 = $row['pros_address'];
		$tampil6 = $row['pros_email'];
		$tampil7 = $row['pros_telp'];
...
$template->display("bodymodal.htm");
