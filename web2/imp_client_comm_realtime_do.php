...
if ($postmode == 'tmp') {
    $unix         = 'TR1609';
    $insert = "INSERT INTO mlm_realtime SET ACCNO = '160712148', Upline = 'COMPANY', datetime  = NOW(), updateby = 'test', rolldate = '0000-00-00', unix = '$unix', is_compress = FALSE";
    $DB->execonly($insert);
    // For Get downline data
    $do = getDataTree('160712148', 'UNIX');
} elseif($postmode == 'tree') {
    $unix         = 'TR1609';
    $periode_date = '2016-06-01 - 2016-06-31';
    $query        = "SELECT * FROM mlm_realtime WHERE unix = 'TR1609' AND is_compress = FALSE";
    ...
    $unix         = 'TR1609';
    $periode = '2016-06-01 - 2016-06-31';
    $periode_date = '2016-06-01 - 2016-06-31';
hitungkomisi('160712148', $periode_date, $periode);
}
...
function getDataTree($account, $unix)
{
    ...
    $data   = array();
    foreach ($result as $key => $value) {
        $insert = "INSERT INTO mlm_realtime SET ACCNO = '$value[ACCNO]', Upline = '$value[Upline]', datetime  = NOW(), updateby = 'test', rolldate = '0000-00-00', unix = 'TR1609', is_compress = FALSE";
        $DB->execonly($insert);
        $data[$key] = $value;
    }
    if (count($data) > 0) {
